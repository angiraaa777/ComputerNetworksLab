...
char buff[512]; /*buffer for sending and receiving messages*/
	...
	for( ; ;)
	{
		...
		while(1)
		{
			...
			if(system(buff))
			{
				...
				{
					ch=fgetc(ptr);
					strncat(outbuff, &ch, 1);
				}while(ch!=EOF);
	
				if((w=write(connfd, outbuff, 512))<0)
				{
					printf("Cannot echo back\n");
				} else {
					printf("Message sent to client %s\n", inet_ntoa(cli_addr.sin_addr));
